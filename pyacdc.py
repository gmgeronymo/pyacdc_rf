...
body{margin:0;font-family:"DejaVu Sans Mono","Consolas",monospace;background:linear-gradient(135deg,#0d1117,#121a23);color:var(--text);font-size:16px;line-height:1.4}
.wrap{padding:14px;display:grid;gap:12px;grid-template-columns:2fr 1fr 1fr}
.card{background:var(--panel);border:1px solid var(--line);border-radius:10px;padding:10px}
h3{margin:0 0 8px 0;font-size:18px;color:#cfe7ff}.row{margin:6px 0;color:var(--muted);font-size:15px}.row b{color:var(--text)}
table{width:100%;border-collapse:collapse;font-size:14px}th,td{border-bottom:1px solid #24303d;padding:8px 6px;text-align:right}th:first-child,td:first-child{text-align:left}
...
.cmd{display:flex;gap:8px}input,button,textarea,select{background:#0f151d;color:var(--text);border:1px solid #304055;border-radius:8px;padding:9px 12px;font-size:15px}
button{cursor:pointer}.btns{display:flex;gap:8px;flex-wrap:wrap;margin-top:8px}.foot{color:var(--muted);font-size:14px;margin-top:6px}
#edit_section{display:none}.compact .hide-compact{display:none!important}
@media (max-width:980px){.wrap{grid-template-columns:1fr;padding:10px}.grid2,.grid1{grid-column:1/span 1;grid-template-columns:1fr}.card{padding:10px}table{display:block;overflow:auto;white-space:nowrap;font-size:13px}.cmd{flex-direction:column}input,textarea,select{width:100%;box-sizing:border-box}}
</style></head><body>
...
function fitCanvas(){const cv=document.getElementById('trend');const pr=cv.parentElement;const fw=pr.clientWidth-20||400;cv.style.width=fw+'px';cv.width=fw*2;cv.style.height='220px';cv.height=440;}function tvcModelToResistance(model){const txt=String(model||'').trim().toUpperCase().replace(',','.');if(!txt)return null;const m=txt.match(/A55\s*[-_ ]\s*([0-9]+(?:\.[0-9]+)?)\s*V/);if(!m)return null;const volts=Number(m[1]);if(!Number.isFinite(volts)||volts<=0)return null;return volts*200;}function autoFillImpedances(){const rs=tvcModelToResistance(document.getElementById('cfg_tvc_std_model').value);const rd=tvcModelToResistance(document.getElementById('cfg_tvc_dut_model').value);if(Number.isFinite(rs))document.getElementById('cfg_rstd').value=String(rs);if(Number.isFinite(rd))document.getElementById('cfg_rdut').value=String(rd);}function drawTrend(rows,totalReps){const cv=document.getElementById('trend'),x=cv.getContext('2d');fitCanvas();const w=cv.width,h=cv.height;const m={l:68,r:20,t:18,b:48};const pw=w-m.l-m.r,ph=h-m.t-m.b;x.clearRect(0,0,w,h);x.fillStyle='#e7edf5';x.font='24px DejaVu Sans Mono';x.strokeStyle='#2a3441';x.strokeRect(m.l,m.t,pw,ph);const points=[];let acceptedCount=0;rows.forEach(r=>{const v=Number(r.dif);if(!Number.isFinite(v))return;const rep=Math.max(1,acceptedCount+1);points.push({rep:rep,dif:v,discarded:!!r.discarded});if(!r.discarded)acceptedCount+=1;});const vals=points.map(p=>p.dif).filter(v=>Number.isFinite(v));let ymin=-1,ymax=1;if(vals.length){ymin=Math.min(...vals);ymax=Math.max(...vals);if(ymin===ymax){ymin-=1;ymax+=1;}const pad=(ymax-ymin)*0.1;ymin-=pad;ymax+=pad;}const reps=(Number.isFinite(totalReps)&&totalReps>0)?Math.floor(totalReps):Math.max(1,acceptedCount,points.reduce((mx,p)=>Math.max(mx,p.rep),1));const yToPix=v=>m.t+(ymax-v)*(ph/(ymax-ymin));const xToPix=i=>m.l+((i-1)/(Math.max(1,reps-1)))*pw;for(let k=0;k<=4;k++){const v=ymin+(k/4)*(ymax-ymin);const yy=yToPix(v);x.strokeStyle='#223040';x.beginPath();x.moveTo(m.l,yy);x.lineTo(m.l+pw,yy);x.stroke();x.fillStyle='#93a1b2';x.fillText(v.toFixed(2),8,yy+5);}x.fillStyle='#93a1b2';x.fillText('Dif. RF-AC [µV/V]',10,14);x.fillText('Repeticao',m.l+pw/2-45,h-10);x.strokeStyle='#304055';x.beginPath();x.moveTo(m.l,m.t+ph);x.lineTo(m.l+pw,m.t+ph);x.stroke();const xticks=Math.min(reps,12);for(let t=1;t<=xticks;t++){const idx=Math.round(1+(t-1)*(reps-1)/Math.max(1,xticks-1));const xx=xToPix(idx);x.strokeStyle='#304055';x.beginPath();x.moveTo(xx,m.t+ph);x.lineTo(xx,m.t+ph+6);x.stroke();x.fillStyle='#93a1b2';x.fillText(String(idx),xx-8,m.t+ph+24);}points.forEach(p=>{const xx=xToPix(Math.min(p.rep,reps));const yy=yToPix(p.dif);x.fillStyle=p.discarded?'#ff6b6b':'#3ecf8e';x.beginPath();x.arc(xx,yy,5,0,Math.PI*2);x.fill();});}async function fetchStatus(){try{const r=await fetch('/api/status');const s=await r.json();document.getElementById('freq').textContent=fmt(s.current_frequency);document.getElementById('vdc').textContent=fmt(s.current_vdc);document.getElementById('vac').textContent=fmt(s.current_vac);document.getElementById('wait').textContent=fmt(s.wait_message);document.getElementById('repetition').textContent=s.running&&s.current_repeat&&s.total_repeats?String(s.current_repeat)+'/'+String(s.total_repeats):'-';document.getElementById('status').textContent=fmt(s.status);document.getElementById('running').innerHTML=s.running?'<span class="ok">EM EXECUCAO</span>':'<span class="hl">PARADO</span>';document.getElementById('pvdc').textContent=Number(s.programmed_vdc||0).toFixed(4)+' V';document.getElementById('pvac').textContent=Number(s.programmed_vac||0).toFixed(4)+' V';document.getElementById('nstd').textContent=fmt(s.n_std);document.getElementById('ndut').textContent=fmt(s.n_dut);const cf=(s.current_frequency||'').split(' ')[0];document.getElementById('freq_list').innerHTML=(s.programmed_frequencies_mhz||[]).map(f=>{const l=Number(f).toFixed(0);return(String(Number(f).toFixed(0))===cf)?'<span class="hl">> '+l+' <</span>':l}).join('<br>')||'-';document.getElementById('cycles').innerHTML=(s.cycle_rows||[]).map(c=>row([fmt(c.cycle),c.std===null?'-':Number(c.std).toLocaleString('pt-BR',{minimumFractionDigits:6,maximumFractionDigits:6}),c.dut===null?'-':Number(c.dut).toLocaleString('pt-BR',{minimumFractionDigits:6,maximumFractionDigits:6})])).join('');document.getElementById('results').innerHTML=(s.results_rows||[]).map(rw=>row([Number(rw.dif).toLocaleString('pt-BR',{minimumFractionDigits:2,maximumFractionDigits:2}),Number(rw.delta).toLocaleString('pt-BR',{minimumFractionDigits:2,maximumFractionDigits:2}),rw.discarded?'<span class="bad">DESCARTADO</span>':'<span class="ok">ACEITO</span>'])).join('');document.getElementById('summary').innerHTML=(s.summary_rows||[]).map(rw=>row([Number(rw.frequency_mhz).toFixed(0),Number(rw.mean).toLocaleString('pt-BR',{minimumFractionDigits:2,maximumFractionDigits:2}),Number(rw.std).toLocaleString('pt-BR',{minimumFractionDigits:2,maximumFractionDigits:2})])).join('')||row(['-','-','-']);drawTrend(s.results_rows||[], Number(s.total_repeats||0));}catch(e){document.getElementById('status').textContent='Falha de comunicação com backend'}}
...
document.getElementById('cmd').addEventListener('keydown',async(e)=>{if(e.key==='Enter')await sendCmd()});document.getElementById('cfg_tvc_std_model').addEventListener('input',autoFillImpedances);document.getElementById('cfg_tvc_dut_model').addEventListener('input',autoFillImpedances);const ro=new ResizeObserver(()=>fitCanvas());ro.observe(document.getElementById('trend').parentElement);fetchStatus();loadConfig();setInterval(fetchStatus,700);
</script></body></html>"""
